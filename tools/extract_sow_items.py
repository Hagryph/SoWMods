...
def write_mod_catalog(parsed, cats):
    ...
    rows = []   # (sortkey, category, group, subgroup, tier, record, display) fully sorted
    for p in parsed:
        ...
        rows.append(((0, so, s0, s1, setname, tier_key(p['tier']), p['record']),
                     'GEAR', slot, setname, p['tier'] or '-', p['record'], humanize(p['record'])))
    catmap = [('RUNE', cats['Rune']), ('RUNEGEM', cats['Runegem']), ('GEM', cats['Gem']),
              ('WEAPON', cats['Weapon']), ('ARMOR', cats['Armor'])]
    for cat, lst in catmap:
        for rec in lst:
            rows.append(((1, cat, rec), cat, cat.title(), '-', '-', rec, humanize(rec)))
    rows.sort(key=lambda r: r[0])

    # mod-consumable TSV: category<TAB>group<TAB>subgroup<TAB>tier<TAB>record<TAB>display  (loaded at runtime)
    ...
    with open(catalog, 'w', encoding='utf-8') as f:
        ...
        f.write('# category\tgroup\tsubgroup\ttier\trecord\tdisplay\n')
        for r in rows:
            f.write('\t'.join([r[1], r[2], r[3], r[4], r[5], r[6]]) + '\n')
    ...
    with open(os.path.join(root, 'sow_catalog', 'catalog.md'), 'w', encoding='utf-8') as f:
        ...
        for r in rows:
            ...
            f.write(f'- {disp}  `{rec}`' + (f'  ({tier})' if tier not in ('-', '') else '') + '\n')
